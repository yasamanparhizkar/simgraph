...
All data-related parameters are given by a dictionary called 'data_params', except for 'ind_min' and 'ind_max':
...
You have the option to force your datasets to be equally distributed between the two labels.
...
def random_train_val_balanced(train_num, val_num, ind_min, ind_max, data_params, seed=None):
    """
    Choose random datapoints to form training and validation datasets. The two sets do not overlap.
    Note: since datapoints are selected randomly, their new indices do NOT represent time anymore.
