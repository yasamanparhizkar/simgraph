...
* Use Matlab-CVX (off-the-shelf SDP solver) to minimize the LMNN objective (i.e. objective#3)
* Use factorization and gradient descent to minimize GLR&trace (i.e. objective#1)
* Compare the above two methods: LMNN should work better than GLR&trace
...
def take_train_step(eng, gamma, beta, train_comb, train_num, val_num, data_params, sg_params, gd_opt_params, fig_params, swap_path_jupyter, swap_path_matlab, res_path_cvx, res_path_fact, seed):
    # create training set
    ...
    # minimize GLR&trace by factorization
    ...
    visualize_M(cvx_M, fig_params, idstr, res_path_cvx)
    visualize_M(fact_M, fig_params, idstr, res_path_fact)
    np.save(res_path_cvx+'matrices/finalM_'+idstr, cvx_M)
    np.save(res_path_fact+'matrices/finalM_'+idstr, fact_M)
    
    return train_num, train_data, cvx_M, cvx_delta, fact_M

def take_val_step(train_data, val_num, data_params, cvx_M, fact_M, sg_params, seed, show_edges=False):
    # unpack params
    ...
    # validate cvxlmnn  
    val_acc_1, y_est_1, t_1 = sg.get_acc(cvx_M, train_data['des'], train_data['lbls'], val_data['des'], val_data['lbls'], sg_params, seed, show_edges)

    # validate factobj1
    val_acc_2, y_est_2, t_2 = sg.get_acc(fact_M, train_data['des'], train_data['lbls'], val_data['des'], val_data['lbls'], sg_params, seed, show_edges)
...
def assess_sg_model(eng, gamma, beta, data_params, sg_params, gd_opt_params, rnd_params, fig_params, swap_path_jupyter, swap_path_matlab, res_path_1, res_path_2):
    time0 = time.time()
    ...
    for val_num in val_sizes:
        ...
        for train_num in train_sizes:
            ...
            for train_comb in range(train_its):
                time3 = time.time()
                # train
                train_num, train_data, cvx_M, cvx_delta, fact_M = \
                take_train_step(eng, gamma, beta, train_comb, train_num, val_num, data_params, sg_params, gd_opt_params, fig_params, swap_path_jupyter, swap_path_matlab, res_path_1, res_path_2, seed)

                for val_comb in range(val_its):
                    # validate
                    val_num, val_data, assess_qs_1, y_est_1, assess_qs_2, y_est_2 = \
                    take_val_step(train_data, val_num, data_params, cvx_M, fact_M, sg_params, seed)
                    # log resutls
