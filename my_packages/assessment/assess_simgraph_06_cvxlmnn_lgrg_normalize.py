"""
* Branched from assess_simgraph_03_gdpaobj2_lgrg.py
...
def take_train_step(eng, lgrg, gamma, beta, train_comb, train_num, val_num, data_params, sg_params, sg_opt_params, fig_params, swap_path_sg, matlab_data_path, res_path_sg, res_path_lgrg, seed):
    # create training set
    # time0 = time.time()
    ...
    savemat(swap_path_sg+'data_'+idstr+'.mat', {'data_feature': train_data['des'], 'data_label': train_data['lbls'], 'data_edge': sg_params['edges_tt']})
    print('-> Iteration ID: ', idstr)
    # fit the graph
    #(M, delta) = eng.lmnn_fit(idstr, gamma, beta, nargout=2) # gdpa-lmnn
    (M, delta, cvx_status) = eng.lmnn_cvx_python(matlab_data_path, idstr, gamma, beta, nargout=3)
    ...
    np.save(res_path_sg+'matrices/finalM_'+idstr, M)
    # print('visualizing and saving M took {} sec'.format(time.time()-time0))
    
    return train_num, train_data, M, delta, lgrg
...
def assess_sg_model(eng, gamma, beta, data_params, sg_params, sg_opt_params, rnd_params, fig_params, swap_path_sg, matlab_data_path, res_path_sg, res_path_lgrg):
    time0 = time.time()
    ...
    for val_num in val_sizes:
        ...
        for train_num in train_sizes:
            ...
            for train_comb in range(train_its):
                time3 = time.time()
                # train
                train_num, train_data, M, delta, lgrg = \
                take_train_step(eng, lgrg, gamma, beta, train_comb, train_num, val_num, data_params, sg_params, sg_opt_params, fig_params, swap_path_sg, matlab_data_path, res_path_sg, res_path_lgrg, seed)
...
def plot_curves(rnd_params, sg_params, feature_id, res_path):
    # unpack params
    train_sizes = rnd_params['train_sizes']
    val_sizes = rnd_params['val_sizes']
    train_its = rnd_params['train_its']
    ...
    for i in range(len(val_sizes)):
        ...
        _ = plt.title('{}, Dt = {}, Dvt = {}, Dv = {}'.format(feature_id, sg_params['Dt'], sg_params['Dvt'], sg_params['Dv']))
...
def plot_curves_without_runtime(rnd_params, sg_params, feature_id, res_path):
    # unpack params
    train_sizes = rnd_params['train_sizes']
    val_sizes = rnd_params['val_sizes']
    train_its = rnd_params['train_its']
