...
New Version Changes: 
  ...
  X 2. Loads feature vectors from disc instead of RAM, therefore less RAM is used (specially useful when all 297 movie repeats are considered).
...
def get_edges_tt(x, Dt=None, seed=None, method='random', times=None):
    """
    ...
    if Dt is None:
        ...
        return get_edges_tt_time(x, Dt, seed, times)
    elif method == 'random':
...
def get_edges_tt_time(x, Dt, seed, times):
    """
    times - Nx1 vector showing the time index of each datapoint
    Refer to documentation for 'get_edges_tt'
    """
    ...
    for i in range(N):
        ...
        if (Dt-degree[i]) < len(candids):
            candids = np.array(sorted(candids, key=lambda x: np.abs(times[x]-times[i])))
            temp = candids[:int(Dt-degree[i])]
        else:
            temp = np.array(candids)
...
def get_edges_vv(num_val, Dv=None, seed=None, method='random', times=None):
    """
    ...
    x = np.arange(num_val)
    return get_edges_tt(x, Dv, seed, method, times)
...
def fit_graph(dess, lbls, opt_params, mu = 0, Dt = None, seed = None, edges_tt = None, edge_method='random', times=None):  
    """
    ...
    if edges_tt is None:
        edges_tt = get_edges_tt(lbls, Dt, seed, edge_method, times)
...
def extrapolate(B, train_des, val_des, train_gt, edges_vv, edges_vt, selective=False, thresh=30):
    """
...
def get_acc(B, train_des, train_y, val_des, val_y, Dv=None, Dvt=None, selective=False, thresh=30, seed=None, show_edges=True, vv_edge_method='random', vv_times=None):
    """
    ...
    num_val = len(val_y)
    edges_vv = get_edges_vv(num_val, Dv, seed, vv_edge_method, vv_times)
    ...
    y_est, t = extrapolate(B, train_des, val_des, train_y, edges_vv, edges_vt, selective, thresh)
