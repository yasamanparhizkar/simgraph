"""
* Construct similarity graphs for training and estimation. Various edge selection strategies are provided.
* Fit the training graph using gradient descent minimizing the GLR+trace objective.
* Estimate new labels by minimizing the GLR of the joint training-validation graph. Compute validation accuracy.
* Several visualization utilites are provided.

New modifications: a new time-based edge selection strategy is provided for tt and vv. The last strategy had the hazard of making disconnected graphs.
...
def get_acc(M, train_des, train_y, val_des, val_y, graph_params, seed=None, show_edges=True):
    """
