"""
* Branched from assess_simgraph_02.py 
* Use Cheng's GDPA optimization code to minimize the objective#2 -> faulty: 'unbounded' objective.
* Compare GDPA-obj#2 with logistic from the scikit-learn package
* Save log and draw comparison plots
Note: This implementation does not have a selective edge mechanism. It only considers a full training graph.
...
def take_train_step(eng, lgrg, train_comb, train_num, val_num, data_params, sg_params, sg_opt_params, fig_params, swap_path_sg, res_path_sg, res_path_lgrg, seed):
    # create training set
    # time0 = time.time()
    ...
    savemat(swap_path_sg+'data_'+idstr+'.mat', {'train_des': train_data['des'], 'train_lbls': train_data['lbls']})
    print('-> Iteration ID: {}_{}_{}'.format(val_num, train_num, train_comb))
    M = eng.gdpa_fit(idstr)
    M = np.array(M)
...
def assess_sg_model(eng, data_params, sg_params, sg_opt_params, rnd_params, fig_params, swap_path_sg, res_path_sg, res_path_lgrg):
    time0 = time.time()
    ...
    for val_num in val_sizes:
        ...
        for train_num in train_sizes:
            ...
            for train_comb in range(train_its):
                ...
                take_train_step(eng, lgrg, train_comb, train_num, val_num, data_params, sg_params, sg_opt_params, fig_params, swap_path_sg, res_path_sg, res_path_lgrg, seed)
