...
def take_train_step(eng, gamma, beta, train_comb, train_num, val_num, data_params, sg_params, fig_params, swap_path_jupyter, swap_path_matlab, res_path_sg, res_path_lgrg, seed):
    # create training set
    # time0 = time.time()
    ...
    # fit the graph
    #(M, delta) = eng.lmnn_fit(idstr, gamma, beta, nargout=2) # gdpa-lmnn
    ...
    V = eng.get_objective_variables_ready_python(swap_path_matlab, idstr)
    V = np.array(V)
    ...
    eng.cd(r'../lmnn-offtheshelf/', nargout=0)

    # compute the LMNN loss
    cvx_loss = np.sum(cvx_M * V.T) + beta * np.sum(cvx_delta)
    gdpa_loss = np.sum(gdpa_M * V.T) + beta * np.sum(gdpa_delta)

    # visualize and save learned M
    # time0 = time.time()
    visualize_M(cvx_M, fig_params, train_comb, train_num, val_num, res_path_sg)
    np.save(res_path_sg+'matrices/finalM_'+idstr, cvx_M)
    visualize_M(gdpa_M, fig_params, train_comb, train_num, val_num, res_path_lgrg)
    np.save(res_path_lgrg+'matrices/finalM_'+idstr, gdpa_M)
    # print('visualizing and saving M took {} sec'.format(time.time()-time0))
    
    return train_num, train_data, cvx_M, cvx_delta, cvx_loss, gdpa_M, gdpa_delta, gdpa_loss, num_LP_rowcol, num_LP_full

def take_val_step(train_data, val_num, data_params, cvx_M, gdpa_M, sg_params, seed, show_edges=False):
    # unpack params
    ...
    val_acc_cvx, y_est_cvx, t = sg.get_acc(cvx_M, train_data['des'], train_data['lbls'], val_data['des'], val_data['lbls'], sg_params, seed, show_edges)
    val_acc_gdpa, y_est_gdpa, t = sg.get_acc(gdpa_M, train_data['des'], train_data['lbls'], val_data['des'], val_data['lbls'], sg_params, seed, show_edges)
    # print('sg.get_acc took {} sec'.format(time.time()-time0))

    # compute several assessment quantities
    # time0 = time.time()
    assess_qs_cvx = assessment_quantities(val_data, val_num, y_est_cvx, val_acc_cvx)
    ...
    return val_num, val_data, assess_qs_cvx, y_est_cvx, assess_qs_gdpa, y_est_gdpa
...
def assess_sg_model(eng, gamma, beta, data_params, sg_params, rnd_params, fig_params, swap_path_jupyter, swap_path_matlab, res_path_sg, res_path_lgrg):
    time0 = time.time()
    ...
    train_num_n_rc_res = np.zeros(len(train_sizes))
    train_num_n_rc_err = np.zeros(len(train_sizes))
    train_num_n_full_res = np.zeros(len(train_sizes))
    train_num_n_full_err = np.zeros(len(train_sizes))

    train_num_loss_res_cvx = np.zeros(len(train_sizes))
    train_num_loss_err_cvx = np.zeros(len(train_sizes))
    train_num_loss_res_gdpa = np.zeros(len(train_sizes))
    train_num_loss_err_gdpa = np.zeros(len(train_sizes))
    ...
    for val_num in val_sizes:
        ...
        for train_num in train_sizes:
            ...
            train_comb_n_full = []
            train_comb_loss_cvx = []
            train_comb_loss_gdpa = []
            for train_comb in range(train_its):
                time3 = time.time()
                # train
                train_num, train_data, cvx_M, cvx_delta, cvx_loss, gdpa_M, gdpa_delta, gdpa_loss, num_LP_rowcol, num_LP_full = \
                take_train_step(eng, gamma, beta, train_comb, train_num, val_num, data_params, sg_params, fig_params, swap_path_jupyter, swap_path_matlab, res_path_sg, res_path_lgrg, seed)

                for val_comb in range(val_its):
                    # validate
                    val_num, val_data, assess_qs_sg, y_est_sg, assess_qs_lgrg, y_est_lgrg = \
                    take_val_step(train_data, val_num, data_params, cvx_M, gdpa_M, sg_params, seed)
                    # log resutls
                ...
                # save number of LP iterations in GDPA
                train_comb_n_rc.append(num_LP_rowcol)
                train_comb_n_full.append(num_LP_full)
                # save losses
                train_comb_loss_cvx.append(cvx_loss)
                train_comb_loss_gdpa.append(gdpa_loss)
                # measure runtime
            ...
            train_num_n_rc_res[j] = np.mean(train_comb_n_rc)
            train_num_n_rc_err[j] = np.std(train_comb_n_rc)
            train_num_n_full_res[j] = np.mean(train_comb_n_full)
            train_num_n_full_err[j] = np.std(train_comb_n_full)
            train_num_loss_res_cvx[j] = np.mean(train_comb_loss_cvx)
            train_num_loss_err_cvx[j] = np.std(train_comb_loss_cvx)
            train_num_loss_res_gdpa[j] = np.mean(train_comb_loss_gdpa)
            train_num_loss_err_gdpa[j] = np.std(train_comb_loss_gdpa)
            ...
            with open(res_path_sg+'curves/runtime_'+str(i)+'.txt', 'a') as file:
                ...
                file.write('\n')
            # save losses
            with open(res_path_sg+'curves/trainloss_'+str(i)+'.txt', 'a') as file:
                file.write(str(train_num_loss_res_cvx[j])+'\n')
                file.write(str(train_num_loss_err_cvx[j])+'\n')
                file.write('\n')
            with open(res_path_lgrg+'curves/trainloss_'+str(i)+'.txt', 'a') as file:
                file.write(str(train_num_loss_res_gdpa[j])+'\n')
                file.write(str(train_num_loss_err_gdpa[j])+'\n')
                file.write('\n')
            # save iteration numbers
            with open(res_path_lgrg+'curves/num_LP_rowcol_'+str(i)+'.txt', 'a') as file:
                file.write(str(train_num_n_rc_res[j])+'\n')
                file.write(str(train_num_n_rc_err[j])+'\n')
                file.write('\n')
            with open(res_path_lgrg+'curves/num_LP_full_'+str(i)+'.txt', 'a') as file:
                file.write(str(train_num_n_full_res[j])+'\n')
                file.write(str(train_num_n_full_err[j])+'\n')
                file.write('\n')
            j += 1
...
def plot_curves(rnd_params, title, res_path):
    ...
    for i in range(len(val_sizes)):
        ...
        plt.ylabel('{} val repeats x {} train repeats'.format(val_its, train_its))
        _ = plt.title(title)
    plt.savefig(res_path+'trainloss_curves.png')
    plt.close()
    

def plot_curves_2(rnd_params, title, res_path):
    # unpack params
    train_sizes = rnd_params['train_sizes']
    val_sizes = rnd_params['val_sizes']
    train_its = rnd_params['train_its']
    ...
    # read iteration number curves
    for i in range(len(val_sizes)):
        curves_i = np.loadtxt(res_path+'curves/num_LP_rowcol_'+str(i)+'.txt')
        if i==0:
            curves = curves_i[::2].reshape((1, -1))
            errors = curves_i[1::2].reshape((1, -1))
        else:
    ...
    for i in range(len(val_sizes)):
        curves_i = np.loadtxt(res_path+'curves/num_LP_full_'+str(i)+'.txt')
        if i==0:
            curves += curves_i[::2].reshape((1, -1))
            errors += curves_i[1::2].reshape((1, -1))
        else:
    ...
    for i in range(len(val_sizes)):
        plt.subplot(len(val_sizes), 1, i+1)
        plt.errorbar(train_sizes, curves[i], errors[i])
        plt.legend('number of LPs')
        plt.xlabel('training set size')
        plt.ylabel('{} val repeats x {} train repeats'.format(val_its, train_its))
        _ = plt.title(title)
    plt.savefig(res_path+'num_LPs.png')
    plt.close()