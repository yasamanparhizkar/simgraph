"""
Preprocess data for the graph classifier.
...
### Read a Single Datapoint From File ###
...
def datapoint_torch(index, data_params):
    ...
    In this system, indices only move forward after repeats, so they represent absolute time in a sense.
    Acceptable index range is batch_sz-1 to 1141*297-1, where batch_sz is the number of frames in the video clip which is labeled by a binary number.
    Feature vectors should be stored as .pt files.
...
def datapoint_numpy(index, data_params):
    ...
    In this system, indices only move forward after repeats, so they represent absolute time in a sense.
    Acceptable index range is batch_sz-1 to 1141*297-1, where batch_sz is the number of frames in the video clip which is labeled by a binary number.
    Feature vectors should be stored as .csv files.
...
def datapoint_sift(index, data_params):
    """
    Warning: Only to use with the mnist-sift features!

    Return a single datapoint consisting of (feature vector, label) based on a given index.
    Feature vectors should be stored as .csv files.
...
def get_labels(data_params):
    """
    Default function to read the labels from two given components of data_params: spike_data and group_id.
    ...
    Output: lbls
    lbls - array of the labels of all datapoints in the original dataset
    """
...
### Create/Update a Set Based on Given Sample Indices ###
...
### Randomly Choose a List of Sample Indices ###
...
### Create/Update Random Training and Validation Sets (Autorun) ###
